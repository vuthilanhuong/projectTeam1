...
app.controller("orderDetailCtrl",function($scope,$stateParams,$http){
    ...
    $scope.edit = function(item){
        localStorage.editOrderDetail = JSON.stringify(item);
    };
...
app.controller('editOrderDetailCtrl',function($scope,$http){

    $scope.loadFix = function(){
        $scope.detail_order = JSON.parse(localStorage.editOrderDetail);
        $scope.createAt = new Date($scope.detail_order.CreatAt);
        $scope.status = [
            {name:"Đã xong",value:"1"},
            {name:"Đang vận chuyển",value:"2"},
    ...
    $scope.submitFix = function(data){
        ...
        $http({
            method: 'PUT',
            url: 'http://localhost:3000/_api/v1/orderDetail/' + data._id,
            data: data
        }).then(function mySuccess(response){
            console.log(response);
            swal({
              ...
              buttons: false
                ...
                window.location.href = "index.html#!/order/"+data.orderId+"/page/1";
            });
        }, function myError(response) {
            console.log('Update loi');
        });
